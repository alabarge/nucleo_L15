...
//   int len = 0;
   ...
   if (path[0] == bk && path[1] == bk)
   {
      ...
      size_t len = 2 + _tcscspn(path+2, invalidChars);
      TCHAR const * end = path+len;

      // server only, no backslash
      if (*end == 0) 
         return GRT_Return(rtServerOnly, (int)len, pLen);

      // server only, terminated with backslash
      if (*end == bk && end[1] == 0) 
         return GRT_Return(rtServerOnly, (int)(len+1), pLen); 

      // server, backslash, and more...
      if (*end == bk)
      {
          if (!greedy)  // return server only
              return GRT_Return(rtServer, (int)len, pLen);
         ...
         if (*end == 0) 
            return GRT_Return(rtServerShare, (int)len, pLen); 

         // server, share, backslash
         if (*end == '\\') 
            return GRT_Return(rtServerShare, (int)(len+1), pLen);
      }
      // fall through to other tests
   }

   size_t len = _tcscspn(path, invalidChars);
   TCHAR const * end = path + len;
   ...
   {
      if (end[1] == '/' && end[2] == '/') 
         return GRT_Return(rtProtocol, (int)(len+3), pLen);
      else 
         return GRT_Return(rtPseudoProtocol, (int)(len+1), pLen); 
   }
...
CPath & CPath::Append(LPCTSTR rhs)
{
    ...
    {
        size_t rhsLen = rhs ? _tcslen(rhs) : 0;
        CStringLock buffer(m_path, (int)m_path.GetLength()+(int)(rhsLen+1));
...
CPath & CPath::RenameExtension(LPCTSTR newExt)
{
    ...
    int maxLen = (int)(m_path.GetLength() + _tcslen(newExt) + 1);
...
CPath CPath::RelativePathTo(LPCTSTR pathTo,bool srcIsDir)
{
    ...
    int maxLen = (int)(3*m_path.GetLength() / 2 +1  + _tcslen(pathTo)); 
